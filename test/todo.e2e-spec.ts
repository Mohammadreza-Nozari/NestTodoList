...
import { CreateTodoContract } from '../src/todo/contracts/create-todo.contract';
import { IRegisterContract } from '../src/auth/contracts/auth.contract';

let userToken: string;
let user: IRegisterContract = {
...
describe('TODO', () => {
  const todo: CreateTodoContract = {
    title: 'New task',
    completed: false,
    category: 'default',
  };

  const todo2: CreateTodoContract = {
    title: 'New task 2',
    completed: false,
    category: 'work',
  };
  ...
  it('should list all todos: []', () => {
    return request(app).get('/todo').expect([]).expect(HttpStatus.OK);
  });
